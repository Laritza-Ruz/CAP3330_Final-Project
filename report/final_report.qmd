...
knitr::kable(
  top10[, c(
    "rank", "seller_id", "seller_order_count", "valid_delivery_count",
    "late_delivery_rate", "canceled_or_unavailable_rate",
    "median_delivery_delay_days", "priority_score"
  )],
  col.names = c(
    "Rank", "Seller ID", "Seller-order records", "Valid deliveries",
    "Late rate", "Cancel/unavail. rate", "Median delay (days)", "Priority score"
  ),
  digits = 3
)
```

```{r seller-plot, fig.cap="Top seller prioritization scores.", fig.align='center', echo=FALSE}
...
| :--- | :--- | :--- | :--- |
| Structure | Create Level 2 Quarto website skeleton | `_quarto.yml`, page scaffolds | Accepted layout; renamed Skill 3 after reviews file was missing |
| Data audit | Inspect CSV grains and join risks | `dataset.qmd`, import/clean drafts | Accepted pre-aggregation concept; added stronger validation later |
| Skill scaffolds | Create six `SKILL.md` files | `skills/*/SKILL.md` | Accepted format; rejected simulated review scores |
...
```powershell
& "C:\Scale Up_Project\Original working Scale Up AI Manager\bin\quarto.exe" render report/final_report.qmd
```

Each Quarto page documents one skill; processed CSV/RDS outputs in `output/` and `data/processed/` support audit trails. The tool is intended for local or intranet deployment as a decision-support dashboard, not autonomous order routing.
...
3. **Implementation limitation:** Analyses use base R for stability; the website excludes the final report from default site render (`!report/final_report.qmd` in `_quarto.yml`) to keep PDF compilation separate. Plot regeneration depends on running analysis scripts during report render.
